...
By default, [`useStore()`](/docs/(qwik)/components/state/index.mdx#usestore) tracks only the `top-level` properties in your store. This means that, in order for an update to be registered, you must modify values at the top-level property.
...
```ts
// `nested` is a top-level property, and object itself, which will be replaced with a new object
...
export default component$(() => {
  const store = useStore(
    ...
    { deep: true } // no recursion `depth limit`
  );
