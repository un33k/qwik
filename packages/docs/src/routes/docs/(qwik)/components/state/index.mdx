...
By default, [`useStore()`](/docs/src/routes/docs/(qwik)/components/state/index.mdx#usestore) tracks only the `top-level` properties in your store. This means that, in order for an update to be registered, you must modify values at the top-level property.
